...
if luci.sys.call("pidof koolproxy >/dev/null") == 0 then
	status = translate("<strong><font color=\"green\">KoolProxy 已启动</font></strong>")
else
	status = translate("<strong><font color=\"red\">KoolProxy 未启动</font></strong>")
end
...
e.description = translate("这些已加入的网站将不使用过滤器。请输入网站的域名，每行只能输入一个网站域。例如google.com。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的网站将使用过滤，但只有黑名单模式。请输入网站的域名，每行只能输入一个网站域名。例如google.com。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的IP地址将使用代理，但只有gfw模型。请输入IP地址或IP地址段，每行只能输入一个IP地址。例如，112.123.134.145/24或112.123.134.145。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已加入的IP地址将不使用广告过滤端口。请输入IP地址或IP地址段，每行只能输入一个IP地址。例如，112.123.134.145/24或112.123.134.145。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
