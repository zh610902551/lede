...
f = SimpleForm("rtorrent", translate("Add Torrent"), '<input class="cbi-button mar-10" type="button" style="text-decoration:underline" value="' .. translate("Return") .. '" onclick="window.location.href=\'./main\';" />')
f.submit = translate("Add")
--f.reset = translate("Return")
...
function uri.validate(self, value, section)
	if "magnet:" == string.sub(value:trim(), 1, 7) then
		...
		if not ok then return nil, translate("Not able to download torrent: ") .. res end
		local tab, err = bencode.decode(res)
		if not tab then return nil, translate("Not able to parse torrent file: ") .. err end
		torrent = res
	end
	return value
end
